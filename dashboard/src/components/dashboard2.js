...
const CovidDashboard = () => {
  ...
  // --- Define the metric options (adjust as needed) ---
  const metricOptions = [
  ...
  useEffect(() => {
    ...
    d3.csv("/owid-covid-data.csv")
      .then((data) => {
        // Convert date strings into Date objects and numeric values
        data.forEach((d) => {
        ...
        setSelectedDate(minDate.getTime())
        ...
        calculateGlobalStats(data, maxDate)
  ...
  // --- Update country data when country or date changes ---
  useEffect(() => {
  ...
  // --- Calculate global statistics ---
  const calculateGlobalStats = (data, latestDate) => {
    // Filter for the latest date and global data
    const latestData = data.filter((d) => d.date.toISOString().slice(0, 10) === latestDate.toISOString().slice(0, 10))

    // Get world data if available, otherwise aggregate
    const worldData = latestData.find((d) => d.location === "World")
    ...
    if (worldData) {
      ...
      const totalCases = d3.sum(latestData, (d) => d.total_cases || 0)
      const totalDeaths = d3.sum(latestData, (d) => d.total_deaths || 0)
      const totalVaccinations = d3.sum(latestData, (d) => d.total_vaccinations || 0)
  ...
  // --- Helper: Get metric color ---
  const getMetricColor = (metric) => {
    const found = metricOptions.find((opt) => opt.value === metric)
    return found ? found.color : "#6b7280"
  }

  // --- Helper: Get metric icon ---
  ...
  const prepareComparisonData = (countries, metric) => {
    ...
    return countries.map((country) => {
      const countryData = csvData.find((d) => d.location === country && d.date.toISOString().slice(0, 10) === selStr)
  ...
  useEffect(() => {
    ...
    // Create a more visually appealing color scale
    ...
    g.selectAll("path")
          ...
          .html(`
            <div class="bg-white p-3 rounded-lg shadow-lg border border-gray-200">
              ...
              <div class="flex items-center mt-1 text-gray-700">
                ...
                <span class="ml-2 font-semibold">${formatValue(val)}</span>
              </div>
            </div>
          `)
      ...
      .on("click", (event, d) => {
        ...
        // Check if this country exists in our dataset
        if (countryList.includes(mappedName)) {
          setSelectedCountry(mappedName)

          // Scroll to country section
          document.getElementById("country-section").scrollIntoView({
            behavior: "smooth",
          })
        }
      })
  ...
  const topCountries = getTopCountries(metricData)
  const comparisonData = prepareComparisonData(
    topCountries.map((c) => c.country),
    selectedMetric,
  )
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {showPurpose && (
            <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200 text-blue-800">
              ...
              <p className="mb-2">
                This dashboard serves as a comprehensive tool for public health officials, policymakers, and researchers
                to monitor and analyze the global impact of COVID-19. It provides real-time data visualization of case
                numbers, deaths, vaccinations, and other key metrics across countries and time periods.
              </p>
              <p>
                By offering interactive features and detailed country-specific analysis, this dashboard enables users to
                identify trends, compare regional impacts, and make data-driven decisions for pandemic response
                strategies. The visualizations are designed to communicate complex epidemiological data in an accessible
            ...
            {showQueries && (
              <div className="mt-2 p-4 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <ul className="list-disc pl-5 space-y-2">
                  ...
                  <li>Which countries have been most severely affected by the pandemic?</li>
                  <li>What is the relationship between vaccination rates and case/death numbers?</li>
                  <li>How do hospitalization rates correlate with mortality rates across different regions?</li>
                  <li>What are the temporal patterns in case surges and declines across different countries?</li>
                  <li>How effective have different countries been in controlling the spread of COVID-19?</li>
                </ul>
              </div>
            )}
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
              ...
              <div className="bg-white rounded-lg shadow p-5 border-l-4 border-red-500">
                <div className="flex justify-between items-start">
                  ...
                  <div
                    className={`flex items-center text-sm ${globalStats.totalCases > 0 ? "text-red-500" : "text-green-500"}`}
                  >
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <h3 className="mt-1 text-2xl font-bold text-gray-900">
                      {formatValue(globalStats.totalVaccinations)}
                    </h3>
                  </div>
                  <div className="flex items-center text-sm text-green-500">
                    <ArrowUp className="w-4 h-4" />
                    <span className="ml-1">
                      {formatPercent((globalStats.totalVaccinations / (globalStats.totalCases || 1)) * 100)}
                    </span>
                  </div>
                </div>
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <div
                    className={`flex items-center text-sm ${kpiChange.increase ? "text-red-500" : "text-green-500"}`}
                  >
            ...
            <div className="bg-white rounded-lg shadow-md mb-8 p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">
                Top 5 Countries by {getMetricLabel(selectedMetric)}
              </h2>
              ...
              <div className="mt-4 text-sm text-gray-500 flex items-center">
                ...
                <span>
                  Data shown for selected date: {selectedDate ? new Date(selectedDate).toLocaleDateString() : ""}
                </span>
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      ...
                      <h3 className="mt-1 text-xl font-bold text-gray-900">
                        {formatValue(countryData[countryData.length - 1]?.total_cases)}
                      </h3>
                      <p className="mt-1 text-xs text-gray-500">
                        {formatValue(countryData[countryData.length - 1]?.total_cases_per_million)} per million
                      </p>
                    </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      ...
                      <h3 className="mt-1 text-xl font-bold text-gray-900">
                        {formatValue(countryData[countryData.length - 1]?.total_deaths)}
                      </h3>
                      <p className="mt-1 text-xs text-gray-500">
                        {formatValue(countryData[countryData.length - 1]?.total_deaths_per_million)} per million
                      </p>
                    </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      ...
                      <h3 className="mt-1 text-xl font-bold text-gray-900">
                        {formatValue(countryData[countryData.length - 1]?.total_vaccinations)}
                      </h3>
                      <p className="mt-1 text-xs text-gray-500">
                        {formatValue(countryData[countryData.length - 1]?.people_vaccinated_per_hundred)}% of population
                      </p>
                    </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      ...
                      <h3 className="mt-1 text-xl font-bold text-gray-900">
                        {formatValue(countryData[countryData.length - 1]?.hosp_patients)}
                      </h3>
                      <p className="mt-1 text-xs text-gray-500">
                        {formatValue(countryData[countryData.length - 1]?.hosp_patients_per_million)} per million
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      <h3 className="text-lg font-medium text-gray-800 mb-4">
                        {getMetricLabel(selectedMetric)} Trend (Last 30 Days)
                      </h3>
                      <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
                    ...
                    {/* Cases vs Deaths Chart */}
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      <h3 className="text-lg font-medium text-gray-800 mb-4">Cases vs Deaths Distribution</h3>
                      <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              data={[
                                {
                                  ...
                                  value: Math.max(
                                    0,
                                    (countryData[countryData.length - 1]?.total_cases || 0) -
                                      (countryData[countryData.length - 1]?.total_deaths || 0),
                                  ),
                                },
                                {
                                  name: "Deaths",
                                  value: countryData[countryData.length - 1]?.total_deaths || 0,
                                },
                              ]}
                              cx="50%"
                              cy="50%"
                            ...
                            >
                              {[
                                { name: "Active/Recovered", color: "#60a5fa" },
                                { name: "Deaths", color: "#6b7280" },
                              ].map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={entry.color} />
                              ))}
                            </Pie>
                            <Tooltip formatter={(value) => [formatValue(value), "Count"]} />
                            <Legend />
                          </PieChart>
                        </ResponsiveContainer>
                      </div>
                      ...
                      <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
                          ...
                          >
                            ...
                            <Tooltip formatter={(value) => [`${value.toFixed(2)}%`, "Percentage of Population"]} />
                            <Bar dataKey="value" name="Percentage of Population" animationDuration={1000}>
                              {[
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <p className="font-semibold">
                          {formatValue(countryData[0]?.hospital_beds_per_thousand)} per 1,000
                        </p>
                      </div>
                      <div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="mb-4 md:mb-0">
              ...
              <p className="text-xs text-gray-400 mt-1">
                Last updated: {dateRange.max ? dateRange.max.toLocaleDateString() : ""}
              </p>
            </div>
            <div className="flex items-center">
              <button
